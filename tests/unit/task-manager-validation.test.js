...
describe('TaskManager Validation Tests', () => {
  ...
  describe('Title validation', () => {
    ...
    test('should reject empty title', async () => {
      await expect(taskManager.createTask({ title: '' })).rejects.toThrow('Title cannot be empty');
    });

    test('should reject whitespace-only title', async () => {
      await expect(taskManager.createTask({ title: '   ' })).rejects.toThrow(
        'Title cannot be empty'
      );
    });

    test('should reject non-string title', async () => {
      await expect(taskManager.createTask({ title: 123 })).rejects.toThrow('Title cannot be empty');

      await expect(taskManager.createTask({ title: null })).rejects.toThrow('Title is required');

      await expect(taskManager.createTask({ title: undefined })).rejects.toThrow(
        'Title is required'
      );
    });
  });
