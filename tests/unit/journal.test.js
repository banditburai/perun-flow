...
describe('Journal Unit Tests', () => {
  ...
  describe('specific logging methods', () => {
    ...
    test('should log note addition with preview', async () => {
      const longNote =
        'This is a very long note that exceeds 100 characters and should be truncated when logged to the journal for preview purposes';
  ...
  describe('query', () => {
    ...
    test('should filter by date range', async () => {
      ...
      expect(
        results.every(
          e => new Date(e.timestamp) >= oneHourAgo && new Date(e.timestamp) <= oneHourFromNow
        )
      ).toBe(true);
    });
  ...
  describe('performance and large data', () => {
    ...
    test('should handle concurrent logging operations', async () => {
      const promises = Array(50)
        .fill()
        .map((_, i) => journal.logOperation(`concurrent.${i}`, { index: i }));
