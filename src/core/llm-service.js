...
export class TaskDecompositionService {
  ...
  _analyzeTaskWithReasoning(task) {
    ...
    if (
      task.title.match(/^(fix|update|change|rename|delete|remove)\s+\w+/i) &&
    ...
    if (
      task.title.toLowerCase().includes('pre-commit') ||
      ...
      return {
        ...
        reasoning:
          'This task involves multiple phases: research, setup, configuration, and validation',
        confidence: 0.9,
      };
    }
   ...
   * @private
   */
  _decomposeWithReasoning(task, _options = {}) {
    // For "Add pre-commit hooks for code quality, style and linting"
      ...
      return [
        {
          ...
          description:
            'Evaluate pre-commit frameworks (pre-commit, husky, lint-staged) and choose appropriate linting tools (eslint, prettier, etc.)',
          priority: 'high',
        },
        {
          title: 'Configure ESLint and Prettier',
          description:
            'Set up ESLint configuration with appropriate rules and Prettier for code formatting',
          priority: 'medium',
        },
        {
          title: 'Install and configure pre-commit framework',
          description:
            'Install chosen pre-commit framework and create configuration file with hooks',
          priority: 'medium',
        },
        {
          title: 'Test and validate pre-commit hooks',
          description:
            'Test the pre-commit hooks with various code scenarios and ensure they work correctly',
          priority: 'high',
        },
      ];
    }
    ...
    if (
      task.title.toLowerCase().includes('authentication') ||
   ...
   * @private
   */
  _generateComplexityReasoning(score) {
    if (score > 0.8) {
